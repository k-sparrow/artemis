"""Contract tests: Debezium CDC → artemis.celery.ingested_objects.
...
into the ingested_objects table-sink topic consumed by its Debezium JDBC
sink connector.

Epic 22 (2026-08-31) retired the second output topic this file used to also
cover, artemis.celery.ingestion_tasks (and its ksqlDB fan-out B/C) — the
storage service now reads ingestion_status directly for task-state
visibility instead of a CDC-fed copy. See
...
Uses JSON Schema (JSON_SR) with Confluent wire format (5-byte header).
...
# ---------------------------------------------------------------------------
# Contract: delete_document tombstone on ingested_objects
# ---------------------------------------------------------------------------
...
class TestDeleteDocumentContract:
    ...
    (Epic 22 retired this class's former ingestion_tasks-recording tests —
    task-state visibility, including delete outcomes, is now a direct read
    of ingestion_status by the storage service, not a second CDC-fed topic.)
    """
