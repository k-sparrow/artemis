...
def create_app():  # noqa: C901
    ...
    if docling_serve_settings.eng_kind == AsyncEngine.RAY:
        ...
        # Local patch (not upstream): QueueLimitExceededError (raised by
        # RayOrchestrator.enqueue when DOCLING_SERVE_ENG_RAY_ENABLE_QUEUE_LIMIT_REJECTION
        # is on and a tenant's queue is full) has no registered handler in
        # stock docling-serve as of v1.29.0-v1.31.0 -- it propagates as an
        # unhandled 500, not the documented 429 (.env.example's own
        # "Return 429 if queue limit exceeded" comment). Confirmed live
        # against the real v1.29.0-based image: 17/20 concurrent requests at
        # MAX_QUEUED_TASKS=3 all came back 500 "Internal Server Error" with a
        # raw traceback in the logs, zero 429s. Filed upstream as
        # docling-project/docling-serve#581; a fix exists
        # (docling-project/docling-serve#583) but is unmerged, unreviewed,
        # and additionally depends on a SystemCapacity class that doesn't
        # exist in any released docling-jobkit -- too large/unstable a
        # surface to backport wholesale, so this mirrors just the handler
        # shape from #583 (itself mirroring RedisBackpressureError's
        # existing pattern above), not the rest of that PR's capacity-endpoint
        # feature.
        @app.exception_handler(QueueLimitExceededError)
        async def queue_limit_exceeded_error_handler(
            request: Request, exc: Exception
        ) -> JSONResponse:
            del request
            return JSONResponse(
                ...
                content={
                    "detail": build_public_http_detail(
                        exc=exc,
                        debug_enabled=docling_serve_settings.debug_error_details,
                        fallback_message="Queue limit exceeded, please try again shortly.",
                    )
                },
