...
#
# Single warm converter replica on a single, unscaled ray-worker node —
# deliberately, not a placeholder. A 2-replica/2-node config (`--scale
# ray-worker=2` + max_replicas_per_node=1) was tried and reverted: it
# requires every deployment to remember to scale ray-worker, and a forgotten
# --scale in the field left Ray Serve permanently stuck trying to schedule
# an unplaceable 2nd replica (ServeController: "1 replicas that have taken
# more than 30s to be scheduled...", looping forever), docling-serve never
# turning healthy, and every dependent service wedged behind it. Revisit
# actor/worker scaling as its own deliberate piece of work, separate from
# the task-timeout and queue-backpressure fixes this block also carries.
...
_DOCLING_RAY_ENGINE_ENV_BLOCK = _DOCLING_RAY_ENGINE_BASE + _DOCLING_RAY_ENGINE_SUFFIX
