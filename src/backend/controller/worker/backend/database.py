"""SQLAlchemy result store backend patch.
Replaces the result column type with LargeBinary instead of PickleType.
"""
...
from src.backend.controller.worker.backend.models import (
    Task,
    TaskExtended,
    TaskSet,
)
...
class DatabaseBackend(BaseBackend):
    ...
    def __init__(self, dburi=None, engine_options=None, url=None, **kwargs):
        ...
        logger.debug(
            f"Task class is "
            f"{self.task_cls.__class__.__module__}.{self.task_cls.__qualname__}"
        )
        ...
        logger.error(f"Configuring database tables with tablenames: {tablenames}")
    ...
    def _create_tables(self):
        ...
        logger.info("Creating MY database tables for result backend.")
        self.ResultSession()
    ...
    def _update_result(self, task, result, state, traceback=None, request=None):
        logger.debug(
            f"[DB DEBUG] _update_result called with "
            f"task_id={task.task_id}, result={result}"
        )
        logger.debug(
            f"[DB DEBUG] _update_result called with type of result {type(result)}"
        )
        ...
        logger.debug(
            f"[DB DEBUG] _update_result enconding "
            f"result={meta['result']}, typeof result={type(meta['result'])}"
        )

        # encode the result into string
        meta["result"] = self.encode(meta["result"])
        logger.debug(
            f"[DB DEBUG] _update_result encoded "
            f"result={meta['result']}, typeof result={type(meta['result'])}"
        )

        # if `self.encode` returns bytes, decode it to string
        if isinstance(meta["result"], bytes):
            logger.debug(f"[DB DEBUG] decoding bytes result={meta['result']} as string")
            meta["result"] = meta["result"].decode()
