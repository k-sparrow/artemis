...
from typing import Any, Dict, Iterator, AsyncIterator, List, Optional, Tuple, cast
...
def _first_page_no(doc_meta: "DocMeta") -> Optional[int]:
    """Return the chunk's primary page — the first provenance ``page_no`` across
    its doc items — or ``None`` when provenance is absent."""
    for item in doc_meta.doc_items:
...
class MetaExtractor(BaseMetaExtractor):
    ...
    def extract_chunk_meta(self, file_path: str, chunk: BaseChunk) -> dict[str, Any]:
        ...
        return {
            ...
            "page_no": _first_page_no(doc_meta),
        }
...
class DoclingAPIServeLoader(BaseLoader):
    ...
    async def aload_artifact(
        self,
    ) -> Tuple[List[Document], List[Tuple[int, str]], DoclingDocument]:
        """Convert once, derive everything the parse artifact needs.

        Returns ``(chunk_docs, pages, dl_doc)`` from a single conversion:

        - ``chunk_docs``: structure-aware agnostic chunks (``HybridChunker``),
          each carrying ``source``/``type``/``page_no`` metadata — no ``dl_meta``.
        - ``pages``: ``(page_no, markdown)`` parents via :func:`split_pages`.
        - ``dl_doc``: the lossless ``DoclingDocument`` so the caller can persist a
          private replay cache. Docling types never leave the parsing service.

        Unlike ``alazy_load``, this always chunks regardless of ``export_type`` —
        the artifact needs both the chunks and the page parents together.
        """
        dl_doc: DoclingDocument = await self._converter.aconvert(source=self._file)
        filename, _, _ = self._file
        chunk_docs = [
            Document(
                page_content=self._chunker.contextualize(chunk=chunk),
        ...
        return chunk_docs, pages, dl_doc
